import { queryAttributeClass,queryAttributeByAttributeClassId } from '../services/attribute';
export default {
  //颜色属性维护
    ...
    effects: {
        ...
        *enterstyleattr({ payload }, { call, put }){
            const {data}= yield call(queryAttributeByAttributeClassId,{jsonParam:payload});
            if(data){
            console.log(data);
             for(let i=1;i<=data.dataList.length;i++){
                    data.dataList[i-1].num=i;
